...
class ClassifierFinModel:
    def __init__(self, model: BaseEstimator, args: dict):
    ...
    def fit(self, mod=True):
        # по умолчанию обрезает датасеты по заданным признакам 
        if mod:
    ...
    def predict_proba(self, X_test):
        if hasattr(self.model, "predict_proba"):
            ...
            raise NotImplementedError(f"{self.model.__class__.__name__} не поддерживает predict_proba")

    def score(self, X_scored, y_scored, output_dict=False):
        y_pred = self.predict(X_scored)
    ...
    def get_constant_accuracy(self, y_test, unknown=False, target_name="direction_binary"):
        if isinstance(y_test, pd.Series):
            y = y_test.reset_index(drop=True).to_frame(name=target_name)
        else:
            y = y_test.reset_index(drop=True)

        value_counts = y[target_name].value_counts()

        zeroes = value_counts.get(0, 0)
        ones = value_counts.get(1, 0)
        total = zeroes + ones

        if total == 0:
            return 0.0  # или np.nan / raise ValueError, если нет ни одного элемента

        return (zeroes if unknown else max(zeroes, ones)) / total
    ...
    def get_shap_values(self):
        if hasattr(self.model, "predict"):
            ...
            raise NotImplementedError("SHAP значения не поддерживаются для этой модели")

    def visualise_shap_values(self):
        shap_values = self.get_shap_values()
        shap.summary_plot(shap_values.values, self.X_train)
...
class CatboostFinModel(ClassifierFinModel):
    ...
    def get_top_imp_features(self, n):
        importances = self.model.feature_importances_
        features = self.model.feature_names_
        top = sorted(zip(features, importances), key=lambda x: x[1], reverse=True)[:n]
        for name, imp in top:
            print(f"{name}: {imp:.4f}")
        return [name for name, _ in top]
    

class SVMFinModel(ClassifierFinModel):
    def __init__(self, args):
        super().__init__(svm.SVC, args)

class LRFinModel(ClassifierFinModel):
        pass

class DecisionTreeFinModel(ClassifierFinModel):
        pass

class AdaBoostFinModel(ClassifierFinModel):
        pass

class XGBoostFinModel(ClassifierFinModel):
        pass
