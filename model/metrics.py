...
def true_positive_long(y_pred, close, commission):
    return ((y_pred == 1) & (close.shift(-1) / close > 1 + commission)).sum()

def true_negative_long(y_pred, close, commission):
    return ((y_pred == 0) & (close.shift(-1) / close <= 1 + commission)).sum()

def false_positive_long(y_pred, close, commission):
    return ((y_pred == 1) & (close.shift(-1) / close <= 1 + commission)).sum()

def false_negative_long(y_pred, close, commission):
    return ((y_pred == 0) & (close.shift(-1) / close > 1 + commission)).sum()

def accuracy_long(y_pred, close, commission):
    tp = true_positive_long(y_pred, close, commission)
    fp = false_positive_long(y_pred, close, commission)
    tn = true_negative_long(y_pred, close, commission)
...
def fbeta_metric_short(y_pred, close, commission, beta=0.005):
